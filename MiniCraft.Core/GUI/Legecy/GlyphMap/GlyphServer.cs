...
namespace MiniCraft.Core.GUI
{
    ...
    public class MGlyphManager
    {
        ...
        private MGlyphManager() { }

        /// <summary>
        /// for each render context, there's a default glyph server.
        /// </summary>
        private static readonly Dictionary<IntPtr, MGlyphManager> defaultServerDict = new Dictionary<IntPtr, MGlyphManager>();
        private static readonly object synObj = new object();
        /// <summary>
        /// default server only provides visible ASCII code.
        /// </summary>
        public static MGlyphManager DefaultServer
        {
            get
            {
                ...
                MGlyphManager server;
                var dict = MGlyphManager.defaultServerDict;
                if (!dict.TryGetValue(context, out server))
                {
                    lock (synObj) // the process of creating a glyph serve may take a long time(several seconds), so we need a lock.
                    {
                        if (!dict.TryGetValue(context, out server))
                        {
                            ...
                            server = MGlyphManager.Create(font, charSet, 1024, 1024, 1000);
                            font.Dispose();
        ...
        /// <returns></returns>
        public static MGlyphManager Create(Font font, IEnumerable<char> charset)
        {
        ...
        /// <returns></returns>
        public static MGlyphManager Create(Font font, IEnumerable<char> charset, int maxTextureWidth, int maxTextureHeight, int maxTextureCount)
        {
        ...
        /// <param name="charset"></param>
        /// <returns></returns>
        public static MGlyphManager Create(Font font, IEnumerable<string> charset)
        {
            int maxTextureSize = GetMaxTextureSize();
            return Create(font, charset, maxTextureSize, maxTextureSize, 100);
        }
        ...
        /// <param name="maxTextureCount"></param>
        /// <returns></returns>
        public static MGlyphManager Create(Font font, IEnumerable<string> charset, int maxTextureWidth, int maxTextureHeight, int maxTextureCount)
        {
            if (charset == null || charset.Count() == 0) { return new MGlyphManager(); }
        ...
        private static MGlyphManager Create(List<ChunkBase> chunkList, int maxTextureWidth, int maxTextureHeight, int maxTextureCount)
        {
            ...
            //{
            //    bitmaps[i].Save($"glyph_{i}.png");

            //}
            var bitmaps = Directory.GetFiles()
    ...
    {
        /// <summary>
        /// Create a <see cref="MGlyphManager"/> instance that provides glyph information according to specified character.
        /// 
        /// </summary>
        /// <param name="font"></param>
        /// <param name="charset"></param>
        /// <returns></returns>
        public static MGlyphManager CreateGlyphServer(this Font font, IEnumerable<char> charset)
        {
            return MGlyphManager.Create(font, charset);
        }

        /// <summary>
        /// Create a <see cref="MGlyphManager"/> instance that provides glyph information according to specified characters.
        /// </summary>
        /// <param name="font"></param>
        /// <param name="charset"></param>
        /// <returns></returns>
        public static MGlyphManager CreateGlyphServer(this Font font, IEnumerable<string> charset)
        {
            return MGlyphManager.Create(font, charset);
        }
    }
